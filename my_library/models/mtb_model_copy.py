...
class BertEmbeddingsMTB(Model):
    ...
    @overrides
    def forward(self,  sentences, test,
                label = None) -> Dict[str, torch.Tensor]:
        # pylint: disable=arguments-differ
        logger.info("reach forward")
        # self.bert_model = BertModel.from_pretrained(self._pretrained_archive_path)
        start_logits, end_logits = self.bert_model(torch.stack(sentences))
        logger.info(end_logits)
        logger.info("reach end point")
        exit()
