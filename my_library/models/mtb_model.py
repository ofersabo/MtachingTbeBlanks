...
class BertEmbeddingsMTB(Model):
    ...
    @overrides
    def forward(self,  sentences, locations, test, test_location,
        ...
        for batch_input in range(relation_representation.size(0)):
            ...
            for i in range(relation_representation.size(1)):
                head, tail = locations[0][i][0]['head'], locations[0][i][0]['tail']
                ...
                # concat_represntentions = self.relation_layer_norm(concat_represntentions)
        ...
        indices = Variable(torch.LongTensor([[head, tail]])).to(self.device)
        test_concat = self.extractor(test_proxy, indices)
        ...
        if label is not None:
            ...
            for metric in self.metrics.values():
                metric(scores.squeeze(2), label.squeeze(1))
